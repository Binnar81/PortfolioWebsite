      ...
      <div className="flex flex-col mt-4">
        ...
        {navLinks.map((navLink) => (
          <Link
            key={navLink.name}
            href={navLink.url}
            className="py-2 px-3 flex items-center rounded-lg hover:bg-yellow-500  font-semibold hover:text-primary custom-transition"
            onClick={() => setNavMenu(false)}
          >
            <span className="pr-6 text-2xl text-yellow-400 hover:text-white">{navLink.icon}</span>
            {navLink.name}
          </Link>
        ))}
        {/* Resume Link */}
        <div className="my-4 py-4 flex flex-col border-y border-gray-500/30">
          <Link
            href={resumeLink}
            className="custom-btn btn-secondary-style text-yellow-500 text-1xl hover:text-white"
          >
            Resume
          </Link>
        </div>
        <div className="flex justify-evenly">
          ...
          {socialLinks.map((socialLink) => (
            <Link
              key={socialLink.name}
              href={socialLink.url}
              className="custom-social-btn text-3xl text-yellow-500 hover:text-white"
            >
              {socialLink.icon}
            </Link>
          ))}
