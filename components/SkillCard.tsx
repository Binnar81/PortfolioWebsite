...
const SkillCard = ({ skillCard }: Props) => {
  ...
  return (
    ...
    <div className="w-full max-w-7xl p-3 flex flex-col rounded-xl bg-slate-900 text-center gap-5">
      {/* Card Icon */}
      <div className="w-fit mx-auto mt-4 p-4 rounded-full bg-slate-900 text-3xl text-primary">{skillCard.icon}</div>

      {/* Card Title */}
      <h3 className="mx-auto text-xl sm:text-2xl font-bold uppercase tracking-widest">{skillCard.title}</h3>

      {/* Skill summary */}
      <p className="mx-auto max-w-sm text-gray-300 sm:text-lg">{skillCard.summary}</p>
